...
describe('result presentation', () => {
  ...
  it('explains automatic removable supports as an analyzed recommendation', () => {
    const support = recommendation('support', 'Auto');
    expect(recommendationExplanation(support, recommendations)).toContain('selected orientation');
    expect(recommendationExplanation(support, recommendations)).toContain('override this assessment');
  });

  it('explains a user support override without claiming geometry required it', () => {
    const support = { ...recommendation('support', 'Auto'), matchedRuleIds: ['G12'] };
    expect(recommendationExplanation(support, recommendations)).toContain('You chose');
    expect(recommendationExplanation(support, recommendations)).not.toContain('overhang exposure');
  });
});

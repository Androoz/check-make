...
export function RecommendedKeySettings({ recommendations, notices }: { recommendations: Recommendation[]; notices: CompatibilityNotice[] }) {
  ...
  return <section className="recommended-key-settings result-plan">
    ...
    <div className="recommendation-groups">{groups.map(group => <section className="recommendation-group" key={group.id}><header><h3>{group.label}</h3><p>{group.description}</p></header>{group.recommendations.map(item => <details className="setting-row" key={item.setting}><summary><b>{labels[item.setting]}</b><strong>{String(item.value)}</strong></summary><div><p>{recommendationExplanation(item, recommendations)}</p>{showTechnicalEvidence && <div className="technical-evidence"><small>{item.ruleIds.join(' · ')} · Evidence {item.evidenceLevel} · {item.validationStatus}</small><small>Decision trace: {item.trace.map(entry => `${entry.ruleId} ${entry.state}${entry.conflictsWithFinal ? ' conflict' : ''}`).join(' · ')}</small><small>Interpretation evidence: {item.inputEvidenceIds?.length ? item.inputEvidenceIds.join(' · ') : 'No semantic input required'}</small></div>}</div></details>)}</section>)}</div>
  </section>;
}

...
export function recommendationExplanation(item: Recommendation, recommendations: Recommendation[]): string {
  ...
  if (item.setting === 'support' && String(item.value).toLocaleLowerCase('en-US') === 'auto') {
    if (item.matchedRuleIds.includes('G12')) {
      return 'You chose to use removable supports in the review checklist. Check Make passes automatic support generation to the slicer so it can place support only where the final geometry and process profile require it.';
    }
    return 'The selected orientation still contains measured overhang exposure. Check Make therefore recommends removable supports only where the slicer determines they are needed. You can override this assessment in the review checklist and Check Make will recalculate the plan.';
  }
  return item.reason;
}

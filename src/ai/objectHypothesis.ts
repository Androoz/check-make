...
export function buildObjectHypothesis(model: ModelAnalysis, contextText = '', ai?: AIObjectEvidence): ObjectHypothesis {
  ...
  const relation = interpretSemanticRelations(contextText);
  ...
  const explicitIdentity = relation.identity;
  ...
  if (explicitIdentity) {
    const id = addEvidence('user-statement', `The Context explicitly names “${explicitIdentity.evidence}”.`, explicitIdentity.confidence);
    identity = assertion('identity', 'Object identity', explicitIdentity.value, explicitIdentity.confidence, 'user-stated', [id]);
  }
  ...
  if (relation.function) {
    const source = relation.function.explicit ? 'user-statement' : 'language-match';
    const id = addEvidence(
      source,
      relation.function.explicit
        ? `The Context explicitly states “${relation.function.evidence}”.`
        : `The named object normally ${relation.function.label}.`,
      relation.function.confidence,
    );
    purpose = assertion(
      'purpose', 'Intended purpose', relation.function.label, relation.function.confidence,
      relation.function.explicit ? 'user-stated' : 'hypothesized', [id],
      ['material', 'orientation', 'structure'],
    );
  }
  if (purposeFacet) {

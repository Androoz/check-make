...
export interface ManufacturingIntent {
  ...
  environment: {
    ...
    service: EvidencedValue<'continuous_outdoor' | 'intermittent_outdoor' | 'washed' | 'dishwasher' | 'unknown'>;
  };
  thermal: {
    band: EvidencedValue<Questionnaire['heat']>;
    explicitRangeC: EvidencedValue<{ minimum: number; maximum: number } | null>;
    coldExposure: EvidencedValue<boolean | 'unknown'>;
  };
  mechanical: {
    ...
    loadMagnitude: EvidencedValue<'light' | 'moderate' | 'heavy' | 'quantified' | 'unknown'>;
    wear: EvidencedValue<'sliding' | 'abrasive' | 'unknown'>;
  };
  interface: {
  ...
  pressureExposure: EvidencedValue<'internal' | 'external' | 'unknown'>;
  electricalRequirement: EvidencedValue<'insulating' | 'unknown'>;
  preferences: {
...
export function buildManufacturingIntent(
  ...
  return {
    ...
    thermal: { band: heat, explicitRangeC: explicitRange, coldExposure: unknown('unknown') },
    mechanical: {
      loadMode,
    ...
    pressureExposure: unknown('unknown'),
    electricalRequirement: unknown('unknown'),
    preferences: {
      priority,
    ...
    compatibility: {
      ...
      coldExposed: temperatures.some(value => value < 0),
      washExposed: false,
      wearExposed: false,
    },
    evidence,
...
export function normalizeManufacturingIntent(
  value: ManufacturingIntent | undefined,
  fallback: ManufacturingIntent,
): ManufacturingIntent {
  if (!value) return fallback;
  const saved = value as Partial<ManufacturingIntent>;
  return {
    ...fallback,
    ...saved,
    environment: { ...fallback.environment, ...(saved.environment ?? {}) },
    thermal: { ...fallback.thermal, ...(saved.thermal ?? {}) },
    mechanical: { ...fallback.mechanical, ...(saved.mechanical ?? {}) },
    interface: { ...fallback.interface, ...(saved.interface ?? {}) },
    preferences: { ...fallback.preferences, ...(saved.preferences ?? {}) },
    compatibility: { ...fallback.compatibility, ...(saved.compatibility ?? {}) },
    evidence: Array.isArray(saved.evidence) ? saved.evidence : fallback.evidence,
  };
}

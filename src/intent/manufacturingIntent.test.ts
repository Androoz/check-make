...
import { buildManufacturingIntent, intentFactUsable, normalizeManufacturingIntent } from './manufacturingIntent';
...
describe('manufacturing intent v3.4', () => {
  ...
  it('adds safe defaults when restoring a manufacturing intent saved before v3.7', () => {
    const fallback = localModelAnalysis(model, 'Indoor holder.').manufacturingIntent!;
    const legacy = structuredClone(fallback) as unknown as Record<string, unknown>;
    delete (legacy.environment as Record<string, unknown>).service;
    delete (legacy.thermal as Record<string, unknown>).coldExposure;
    delete (legacy.mechanical as Record<string, unknown>).loadMagnitude;
    delete (legacy.mechanical as Record<string, unknown>).wear;
    delete legacy.pressureExposure;
    delete legacy.electricalRequirement;
    legacy.compatibility = { structural: true, fitCritical: false, flexible: false, weatherExposed: false, heatExposed: false };
    const restored = normalizeManufacturingIntent(legacy as never, fallback);
    expect(restored.environment.service.value).toBe('unknown');
    expect(restored.thermal.coldExposure.value).toBe('unknown');
    expect(restored.mechanical.loadMagnitude.value).toBe('unknown');
    expect(restored.mechanical.wear.value).toBe('unknown');
    expect(restored.pressureExposure.value).toBe('unknown');
    expect(restored.electricalRequirement.value).toBe('unknown');
    expect(restored.compatibility).toMatchObject({ coldExposed: false, washExposed: false, wearExposed: false });
  });
});
